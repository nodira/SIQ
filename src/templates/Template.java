...
public class Template {
	...
	public List<SymbolicTuple> potentialMatches(TemplateTuple template, SimpleView... views){
		...
		for(SimpleView v : views){
			...
			if(v.getViewName().equals(templateTuples[0].underlyingRelation().relationSchema().getRelationName())){
	...
	public String toString(){
		...
		for(TemplateTuple t : templateTuples){
			sb.append(t.underlyingRelation().relationSchema().getRelationName() + "("); 
	...
	public Template clone(){
		...
		for(int i=0; i< templateTuples.length; i++){
			...
			TemplateTuple tClone = new TemplateTuple(t.underlyingRelation()); 
